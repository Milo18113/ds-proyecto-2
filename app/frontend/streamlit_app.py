import os
import requests
import streamlit as st

API_URL = os.getenv("API_URL", "http://localhost:8000")
TIMEOUT = 15

st.set_page_config(page_title="Fintech Mini Bank", layout="wide")
st.title("🏦 Fintech Mini Bank")

def post(path: str, payload: dict):
    r = requests.post(f"{API_URL}{path}", json=payload, timeout=TIMEOUT)
    try:
        data = r.json()
    except Exception:
        data = {"detail": r.text}
    return r.status_code, data

def get(path: str):
    r = requests.get(f"{API_URL}{path}", timeout=TIMEOUT)
    try:
        data = r.json()
    except Exception:
        data = {"detail": r.text}
    return r.status_code, data

tabs = st.tabs(["Cliente", "Cuenta", "Depósito", "Retiro", "Transferencia", "Saldo", "Transacciones"])

with tabs[0]:
    st.subheader("Crear cliente")
    name = st.text_input("Nombre")
    email = st.text_input("Email")
    if st.button("Crear cliente", type="primary"):
        code, data = post("/customers", {"name": name, "email": email})
        st.write("Status:", code)
        st.json(data)

with tabs[1]:
    st.subheader("Crear cuenta")
    customer_id = st.text_input("Customer ID")
    currency = st.text_input("Currency", value="USD")
    if st.button("Crear cuenta", type="primary"):
        code, data = post("/accounts", {"customer_id": customer_id, "currency": currency})
        st.write("Status:", code)
        st.json(data)

with tabs[2]:
    st.subheader("Depósito")
    account_id = st.text_input("Account ID (deposit)")
    amount = st.number_input("Monto", min_value=0.0, step=10.0)
    if st.button("Depositar", type="primary"):
        code, data = post("/transactions/deposit", {"account_id": account_id, "amount": float(amount)})
        st.write("Status:", code)
        st.json(data)

with tabs[3]:
    st.subheader("Retiro")
    account_id = st.text_input("Account ID (withdraw)")
    amount = st.number_input("Monto ", min_value=0.0, step=10.0, key="w_amt")
    if st.button("Retirar", type="primary"):
        code, data = post("/transactions/withdraw", {"account_id": account_id, "amount": float(amount)})
        st.write("Status:", code)
        st.json(data)

with tabs[4]:
    st.subheader("Transferencia")
    from_id = st.text_input("From account")
    to_id = st.text_input("To account")
    amount = st.number_input("Monto  ", min_value=0.0, step=10.0, key="t_amt")
    if st.button("Transferir", type="primary"):
        code, data = post(
            "/transactions/transfer",
            {"from_account_id": from_id, "to_account_id": to_id, "amount": float(amount)}
        )
        st.write("Status:", code)
        st.json(data)

with tabs[5]:
    st.subheader("Ver saldo / cuenta")
    account_id = st.text_input("Account ID (get)")
    if st.button("Consultar", type="primary"):
        code, data = get(f"/accounts/{account_id}")
        st.write("Status:", code)
        st.json(data)

with tabs[6]:
    st.subheader("Listar transacciones")
    account_id = st.text_input("Account ID (tx list)")
    if st.button("Listar", type="primary"):
        code, data = get(f"/accounts/{account_id}/transactions")
        st.write("Status:", code)
        st.json(data)

st.caption(f"API_URL: {API_URL}")